...
local ic1eq, ic2eq = 0, 0

function filter_params(fc, res, mode)
  local fMin, fMax = math.log(20), math.log(20000)
  local cutoff = math.exp(fc * (fMax - fMin) + fMin)
  local g = math.tan(math.pi * cutoff / sampleRate)
  local k = 2 - 2 * res -- 1/Q
  local a1 = 1 / (1 + g * (g + k))
  local a2 = g * a1
  local a3 = g * a2
...
function filter(v0, a1, a2, a3, k, g)
  local v3 = v0 - ic2eq
...
function process(frames)
  -- call cutoff, res, and mode once per frame buffer
  local a1, a2, a3, k, g = filter_params(fc[1], res[1], mode[1])
  for i = 1, frames do
    -- call filter function for every sample
    out[i] = filter(input[i], a1, a2, a3, k, g)
  end
end
